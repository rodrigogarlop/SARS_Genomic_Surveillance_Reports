# Update 2021-05-27 2: The new tables had fewer genes. I added exceptions to avoid errors
# Update 2021-05-27 1: The new tables had empty lines. I added a workaround for this issue.
# Started: 2021-05-27
...
# csv <- "Metadata/MexCoV2_Resul/24Marzo2021_MutationAmino.csv"
...
# Test in bash:
# Rscript CoViGen-Mex_Mutations_Report.R Metadata/MexCoV2_Resul/24Marzo2021_MutationAmino.csv Test2/output2  2021-05-19
...
remove_double_asterisks <- function(inmat){ # This was created to deal with a special case where repeated items are found in historical data, one of which has double asterisks and is the one removed here
	outmat <- inmat
	error <- grep("\\*\\*", rownames(inmat)) # get a vector of rows with double asteriks
	if(length(error)>0){
...
print("The following genes were found:")
print(gene_list)
...
df <- remove_double_asterisks(df) # Comment if required
